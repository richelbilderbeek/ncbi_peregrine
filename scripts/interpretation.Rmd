...
Here, I'll discuss the data and its size.

```{r}
n_membrane_proteins <- nrow(ncbiperegrine::read_genes_ids_file("gene_ids.csv"))
```

There are `r n_membrane_proteins` membrane proteins in our group of membrane-associated proteins.
...
There are `r n_variations` variations, in which a
variation is defined as a change at 
the DNA, RNA or protein level. For example,
the variation `NP_001333827.1:p.Cys1115Gly`
denotes an amino acid substitution.
In this pipeline, we already selected for changes
at the protein level.

However, not all variations are one-nucleotide 
subtitutions. For example, `NP_001333827.1:p.Val1095fs`
denotes a frame shift, that resulted in one or more
changes at the protein level.  We chose to focus
on only one-nucleotide 
subtitutions, as these changes only have a local effect.

To keep only the variations with
one-nucleotide subtitutions, remove the results with NAs:
...
There are `r n_variations` variations with a one-residue 
substitution, of which the varations can be duplicates.
...
What is the chance that we find this number of
amino acids substitutions in our dataset?

Would a variations occur just as likely in TMH
as well as in solvent exposed regions,
the chance it is in a TMH equals the percentage
of its protein being TMH.

Hence, for all variations, 
the **expected** number of variations in TMHs 
can be calculated by summing 
the percentages of the potein they work on being TMH :
...
as in a soluble region of the protein, 
we expect `r expect_n_variations_in_tmhs`
...
What is the chance that this number is observed, 
if the variations would be randomly distributed 
over the proteins:
